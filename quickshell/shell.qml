...
PanelWindow {
    ...
    // Watch root's volume properties — no cross-scope ID refs needed
    Connections {
        target: root
        function onVolumePercentChanged() { volOverlay.pillOpacity = 1; volHideTimer.restart() }
        function onIsMutedChanged()       { volOverlay.pillOpacity = 1; volHideTimer.restart() }
    }
